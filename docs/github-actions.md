...
单账号归档必须在根目录包含一对 `history.json` 与 `account-state.json`；多账号归档必须按启用账号分别放在 `<account-id>/` 目录中，且与本次配置完全一致。单账号和多账号配置切换时，工作流会在发送前拒绝不匹配的归档，不会把一套状态当成另一套状态继续运行。
...
每次正式运行在发送前会保存一个不含敏感数据的不可变发送 marker；如果运行中断或状态 Artifact 上传失败，后续运行会发现未配对 marker 并安全停止，而不是回退到旧 history。遇到这个安全停止时，先检查对应运行日志、通知和状态 Artifact，再决定是否人工恢复；不要直接删除 marker，也不要在无法确认“当天没有发送”时使用 bootstrap。公开仓库的 Artifact 对仓库读者可见，虽然不含登录凭证，仍可能包含好友身份和发送历史元数据，请据此评估仓库可见范围。

人工恢复按以下顺序执行：

1. 记录未配对 marker 对应的运行编号，核对该运行是否进入浏览器、是否有发送成功/未确认通知，并保留失败运行的日志和诊断 Artifact。
2. 只有在确认该运行在发送前就失败、当天没有消息可能已经发出时，才可以在 Actions 页面删除这一枚 marker，然后按普通流程重跑；如果已有有效状态归档，不要使用 `bootstrap_state`。
3. 只要有任何消息可能已经发出，就保留 marker，先从已核实的备份或人工整理结果恢复一套正确的 `history.json` 与 `account-state.json`，确认归档配对后再移除 marker。无法证明状态正确时宁可继续停机，不要绕过安全门。
4. 如果所有状态归档都已过期且能确认当天没有发送，才可显式使用一次 `bootstrap_state` 建立新归档；完成后先确认归档存在，再恢复普通运行。
