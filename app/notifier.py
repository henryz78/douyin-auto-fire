...
async def send_telegram_notification(
    ...
    message = build_telegram_message(task_id, dry_run, results, screenshots, finished_at)
...
def build_telegram_message(
    ...
    lines = [
        title,
        "",
        f"任务：{_telegram_text(task_id, limit=100)}",
        f"模式：{mode}",
        f"完成时间：{finished}",
    ...
    if successes:
        for index, result in enumerate(successes[:MAX_RESULTS_PER_SECTION], 1):
            ...
            lines.append(f"{index}. {_telegram_text(result.target, limit=100)} - {detail}")
    ...
    if failures:
        for index, result in enumerate(failures[:MAX_RESULTS_PER_SECTION], 1):
            ...
            lines.append(f"{index}. {_telegram_text(result.target, limit=100)}{sent}")
    ...
    if screenshots:
        lines.extend(["", "失败截图"])
        lines.extend(f"- {_telegram_text(path.name, limit=100)}" for path in screenshots[:MAX_RESULTS_PER_SECTION])
...
def _post_telegram_message(bot_token: str, chat_id: str, text: str) -> None:
    ...
    try:
        ...
        # The bot token is part of the request URL; never expose the raw
        # transport exception to logs or notifications.
