...
def test_telegram_message_reports_success_and_failure() -> None:
    results = [
        TargetResult(target="好友A", status="success", sent=2),
        TargetResult(target="好友B", status="failed", sent=1, error="发送失败\n请重试"),
    ]
    ...
    assert "抖音自动发送：存在失败" in message
    assert "完成时间：2026-08-09 16:00:00 +0800" in message
    assert "成功名单（1）" in message
    assert "1. 好友A - 已发送 2 条" in message
    assert "失败名单（1）" in message
    assert "1. 好友B，已发送 1 条" in message
    assert "   - 原因：发送失败 请重试" in message
    assert "失败截图" in message
    assert "friend-b.png" in message


def test_telegram_message_redacts_configured_secrets(monkeypatch) -> None:
    monkeypatch.setenv("DOUYIN_COOKIE", "COOKIE_SECRET")
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "BOT_SECRET")
    message = build_telegram_message(
        "daily-streak",
        False,
        [TargetResult(target="好友A", status="failed", error="COOKIE_SECRET BOT_SECRET")],
    )

    assert "COOKIE_SECRET" not in message
    assert "BOT_SECRET" not in message
...
async def test_send_telegram_notification_sends_all_chunks(monkeypatch) -> None:
    results = [
        TargetResult(target=f"好友{index}", status="failed", error="发送失败" * 400)
